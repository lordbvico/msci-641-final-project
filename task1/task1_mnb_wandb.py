...
sweep_id = wandb.sweep(sweep=sweep_configuration, project="MNB Parameter Sweep")
...
def main():   
    ...
    t_pipeline.fit(X_train, train_y)
    y_pred = t_pipeline.predict(X_val)
    score = accuracy_score(val_y, y_pred)
    wandb.log({"Best Validation Accuracy": score, "alpha": alpha})
    if score > maxi:
        maxi = score
        #hyper = i
    ...
    #nb_model = pickle.load(filename)
                  
    #y_pred = nb_model.predict(sample)
    ...
    Out_df.to_csv("output_mnb_uni_bi_wandb.csv", index=False)
