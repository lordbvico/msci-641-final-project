...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        hidden = (
            torch.zeros(num_layers, train_features.size(0), hidden_size).to(device),
            torch.zeros(num_layers, train_features.size(0), hidden_size).to(device)
        )
        for batch_features, batch_labels in tqdm(train_loader):
            ...
            outputs = model(batch_features, hidden)
        ...
        with torch.no_grad():
            ...
            for val_features, val_labels in tqdm(val_loader):
                ...
                # Forward pass with BPTT
                val_outputs = model(val_features, hidden)
    ...
    model_filename = "lstm_model_bptt.pth"
    torch.save(model.state_dict(), model_filename)

    model.load_state_dict(torch.load("lstm_model_bptt.pth"))
    model.eval()
    test_features = test_features.to(device)
    hidden = None  # Initialize hidden state
    test_outputs, _ = model(test_features, hidden)
    ...
    Out_df1.to_csv("output_LSTM_modified_bptt.csv", index=False)
