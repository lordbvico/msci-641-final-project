...
def preprocess_data(df):
   ...
   # df.loc[:, ['postText']] = df.loc[:, ['postText']].astype(str)
   ...
   # df["targetTitle_tokens"] = df.apply(lambda row: [element for element in row["targetTitle_tokens"] if element not in stopwords], axis=1)
    ...
    # df[['postText_tokens']] = df[['postText_tokens']].replace(r'\s+', ' ', regex=True).replace(r'\W', ' ', regex=True)
...
def main():
   ...
   # kernel_sizes = [2, 3, 4]
    ...
    # learning_rate = 0.001
    learning_rate = 0.001174144533976564
   ...
   # num_epochs = 10
    ...
    best_model_filename = "best_model_cnn_lstm_optimized.pth"
    ...
    Out_df1.to_csv("output_cnn_lstm_best_optimized.csv", index=False)
